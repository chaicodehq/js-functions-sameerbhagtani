...
export function createRecipe(steps) {
    if (!Array.isArray(steps) || steps.length === 0) return pipe();
    ...
    const filteredSteps = steps.filter((step) => step in functions);

    return pipe(...filteredSteps.map((step) => functions[step]));
}
